...
function filterStudentsByGrade(students, grade) {
  console.error(
    "Якщо ви бачите це повідомлення, завдання 1 виконано неправильно"
...
warnAboutMaxArrayLength([1, 2, 13, 5, 6], 7);
warnAboutMaxArrayLength("Hello", "World");
...
function compareArrays(arr1, arr2) {
  arr1.sort((a, b) => a - b);
  arr2.sort((a, b) => a - b); // Сортуємо масиви за зростанням.
  // Сортуємо масиви за зростанням.
  const json1 = JSON.stringify(arr1);
  const json2 = JSON.stringify(arr2); // Оскільки напряму порівняти масиви за допомогою оператора === не можно, перетворимо ії в json і порівняємо

  console.assert(json1 === json2, "Масиви неоднакові");
...
function buildMatrix(size) {
  ...
  console.table(matrix); // Створюємо порожній масив, який буде рядком матриці.
  return matrix; // Цикл всередені циклу для заповнення колонок матриці,лічильник j має бути від 0 до розміру матриці.
...
function displayGroupedInfo(students) {
  ...
  // Виведемо корисне повідомлення з текстом `Вік: ${student.name}`
  // Виведемо корисне повідомлення з текстом `Предмет: ${student.age}`
  // Закриємо групу
}
...
validateUserInput({ username: 0, password: "password123" });
...
function calculateTotalPrice(products) {
  console.time("Calculate Total Price"); // Початок вимірювання часу
  ...
  console.timeEnd("Calculate Total Price"); // Зупинка вимірювання та виведення часу
}
...
function countVowelsAndConsonants(word) {
  ...
  for (const letter of word.toLowerCase()) {
    if (vowels.includes(letter)) {
      vowelCount++;
      console.log(`Голосна: ${vowelCount}`);
    } else {
      consonantCount++;
      console.log(`Приголосна: ${consonantCount}`);
    }
